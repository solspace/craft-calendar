...
namespace Solspace\Calendar\Library\ColorJizz;

use Solspace\Calendar\Library\ColorJizz\Formats\CIELab;
use Solspace\Calendar\Library\ColorJizz\Formats\CIELCh;
use Solspace\Calendar\Library\ColorJizz\Formats\CMY;
use Solspace\Calendar\Library\ColorJizz\Formats\CMYK;
use Solspace\Calendar\Library\ColorJizz\Formats\Hex;
use Solspace\Calendar\Library\ColorJizz\Formats\HSV;
use Solspace\Calendar\Library\ColorJizz\Formats\RGB;
use Solspace\Calendar\Library\ColorJizz\Formats\XYZ;
use Solspace\Calendar\Library\ColorJizz\Formats\Yxy;
...
abstract class ColorJizz
{
    ...
    public function distance(self $destinationColor)
    {
        ...
        $lightness_pow = ($a->lightness - $b->lightness) ** 2;
        $a_dimension_pow = ($a->a_dimension - $b->a_dimension) ** 2;
        $b_dimension_pow = ($a->b_dimension - $b->b_dimension) ** 2;
    ...
    public function match(array $palette)
    {
        ...
        for ($i = 0; $i < \count($palette); ++$i) {
            ...
            if (100000000000 == $distance || $cdistance < $distance) {
                $distance = $cdistance;
                $closest = $palette[$i];
            }
        }
